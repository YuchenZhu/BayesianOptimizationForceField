...
import json
import pickle
from utils.post_process import ppSol
...
def tgFunc( sg, ep ):
    ...
    write_forcefield('%s/in.forcefield' %loc ,
             'hybrid lj/cut/tip4p/long/soft 1 2 1 1 0.1546 1 0.5 10.0 %.1f %.1f'%(rc, rc), # functional
    ...
    obs.append([  cnt+1, G, sg , ep ] )
    obsArr = np.array(obs )
    obsErArr = obsArr.copy()
    obsErArr[:, 1] = transform( obsErArr[:, 1], tg)
